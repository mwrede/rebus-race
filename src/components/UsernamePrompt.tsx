...
import { setUsername, setGoogleUser, getGoogleUser, getAnonId, setAnonId } from '../lib/auth';
    ...
    try {
      ...
      await supabase
        ...
        .upsert({
          ...
          email: googleUser.email, // Also set email field
        }, {
          onConflict: 'anon_id'
        });
      ...
      if (!input.trim()) {
        let defaultUsername = googleUser.name || googleUser.email.split('@')[0];
        ...
        if (defaultUsername.length < 2) {
          defaultUsername = googleUser.email.split('@')[0].substring(0, 20);
        }
        setInput(defaultUsername);
      }
      // Don't auto-submit - user still needs to confirm username
    } catch (error) {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="mb-4">
            <p className="text-sm text-gray-600 mb-2">Or sign in with Google (optional):</p>
            <button
              type="button"
              onClick={() => {
                setIsGoogleLoading(true);
          ...
          <button
            type="submit"
            disabled={isChecking || isGoogleLoading}
